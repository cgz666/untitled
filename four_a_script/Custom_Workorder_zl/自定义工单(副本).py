...
class Custom_Workorder():
    ...
    def excel_process(self):
        ...
        try:
            ...
            # ===== 设置公式计算辅助表数据 =====
            for i in range(3, 17):
                # AZ列公式 - 统计Q列包含AM地市且G列状态为'已领取'或'待领取'的工单数量
                sheet_main.Range(f'AZ{i}').Formula = (
                    f'=COUNTIFS(工单信息!Q:Q,"*"&AM{i}&"*",工单信息!G:G,"已领取") + '
                    f'COUNTIFS(工单信息!Q:Q,"*"&AM{i}&"*",工单信息!G:G,"待领取")'
                )

                # AO和AP列公式 - 累计故障接单及时/超时(2小时)，排除AZ条件的数据
                sheet_main.Range(f'AO{i}').Formula = (
                    f'=COUNTIFS(工单信息!Q:Q,"*"&AM{i}&"*",'
                    f'工单信息!AK:AK,"<2",'
                    f'工单信息!G:G,"<>待领取")'
                )
                sheet_main.Range(f'AP{i}').Formula = (
                    f'=COUNTIFS(工单信息!Q:Q,"*"&AM{i}&"*",'
                    f'工单信息!AK:AK,">=2",'
                    f'工单信息!G:G,"<>待领取")'
                )

                # AQ和AR列公式 - 累计处理及时/超时（8小时），排除AZ条件的数据
                sheet_main.Range(f'AQ{i}').Formula = (
                    f'=COUNTIFS(工单信息!Q:Q,"*"&AM{i}&"*",'
                    f'工单信息!AL:AL,"<8",'
                    f'工单信息!G:G,"<>已领取",'
                    f'工单信息!G:G,"<>待领取")'
                )
                sheet_main.Range(f'AR{i}').Formula = (
                    f'=COUNTIFS(工单信息!Q:Q,"*"&AM{i}&"*",'
                    f'工单信息!AL:AL,">=8",'
                    f'工单信息!G:G,"<>已领取",'
                    f'工单信息!G:G,"<>待领取")'
                ...
                # AU和AV列公式 - 当月故障接单及时/超时(2小时)，排除AZ条件的数据
                sheet_main.Range(f'AU{i}').Formula = (
                    f'=COUNTIFS(工单信息!Q:Q,"*"&AM{i}&"*",'
                    f'工单信息!AK:AK,"<2",'
                    f'工单信息!G:G,"<>待领取",'
                    f'工单信息!AD:AD,">="&DATE(YEAR(TODAY()),MONTH(TODAY()),1),'
                    f'工单信息!AD:AD,"<="&EOMONTH(TODAY(),0))'
                )
                sheet_main.Range(f'AV{i}').Formula = (
                    f'=COUNTIFS(工单信息!Q:Q,"*"&AM{i}&"*",'
                    f'工单信息!AK:AK,">=2",'
                    f'工单信息!G:G,"<>待领取",'
                    f'工单信息!AD:AD,">="&DATE(YEAR(TODAY()),MONTH(TODAY()),1),'
                ...
                # AX和AY列公式 - 当月故障处理及时/超时(8小时)，排除AZ条件的数据
                sheet_main.Range(f'AX{i}').Formula = (
                    f'=COUNTIFS(工单信息!Q:Q,"*"&AM{i}&"*",'
                    f'工单信息!AL:AL,"<8",'
                    f'工单信息!G:G,"<>已领取",'
                    f'工单信息!G:G,"<>待领取",'
                ...
                sheet_main.Range(f'AY{i}').Formula = (
                    f'=COUNTIFS(工单信息!Q:Q,"*"&AM{i}&"*",'
                    f'工单信息!AL:AL,">=8",'
                    f'工单信息!G:G,"<>已领取",'
                    f'工单信息!G:G,"<>待领取",'
